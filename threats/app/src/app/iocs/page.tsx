  ...
  return (
    <div className="max-w-[1400px] mx-auto px-8 py-12">
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-light tracking-wider2">INDICATORS OF COMPROMISE</h1>
      </div>
      <div className="flex gap-2 mb-6 text-[11px] tracking-widest2 flex-wrap">
      ...
      <div className="border border-line overflow-x-auto">
        <div className="grid grid-cols-12 text-[10px] tracking-widest2 text-dim border-b border-line bg-panel min-w-[800px]">
          ...
          <div className="col-span-2 p-4">SOURCE</div>
          <div className="col-span-2 p-4 text-right">FIRST SEEN</div>
        </div>
        {iocs.length === 0 ? (
          <div className="p-12 text-center text-dim text-sm">No IOCs ingested yet.</div>
        ) : iocs.map((i: any) => (
          <Link
            key={i.id}
            href={`/ioc/${i.id}`}
          ...
          >
            <div className="col-span-6 p-4 font-mono truncate">{i.value}</div>
            <div className="col-span-2 p-4 text-dim text-[10px] tracking-widest2">{i.type.toUpperCase().replace('_', ' ')}</div>
            <div className="col-span-2 p-4 text-dim text-[10px] truncate">{i.source_name || '—'}</div>
            <div className="col-span-2 p-4 text-right text-dim text-[10px]">{relativeTime(i.first_seen)}</div>
          </Link>
        ))}
      </div>
    </div>
